<template>
  <div class="add headFixed">
    ...
    <div class="list list_normal list_no_arrow">
      <div class="item">
        <div class="clear">
          <p v-show="this.tab==0">使用定时 <span class="right timerSwitch tTimer"><mt-switch v-model="value"
                                                                                         @change="turn()"></mt-switch></span>
          </p>
          <p v-show="this.tab==1">使用定时 <span class="right timerSwitch tTimer"><mt-switch v-model="closeVal"
                                                                                         @change="turnClose"></mt-switch></span>
          </p>
        </div>
      </div>
    </div>
        ...
        <p class="clear">
          ...
          <span class="right timerSwitch">
            ...
            <mt-switch :value="item.status==1?true:false"
                       @change="changOpenVal(item.id,index)"></mt-switch>
            </span>
        </p>
        <p class="time">
          <span>{{item.date}}</span>
      ...
      <div class="list" v-for="(item,index) in close" :key="index">
        <p class="clear">
          ...
          <span class="right timerSwitch">
            ...
            <mt-switch :value="item.status==1?true:false"
                       @change="changOpenVal(item.id,index)"></mt-switch></span>
        </p>
        <p class="time">
          <span>{{item.date}}</span>
        </p>
  ...
  export default {
    ...
    methods: {
      ...
      changOpenVal(id, index, checked) {
        ...
        this.updateTimerTask(id, status);
      },
      turnClose() {
        let arr = [];
